...
exports.listarProdutos = (req, res) => {
  ...
  db.query(sql, (err, results) => {
    if (err) return res.status(500).json({ mensagem: 'Erro ao buscar produtos.', erro: err });
...
exports.cadastrarProduto = (req, res) => {
  const { nome_produto, codigo_barras, descricao, quantidade, categoria } = req.body;

  if (!nome_produto || !codigo_barras || !descricao || !quantidade || !categoria) {
  ...
  db.query(verificarCodigo, [codigo_barras], (err, results) => {
    ...
    const sql = `
      ...
      VALUES (?, ?, ?, ?, ?)
    `;
    db.query(sql, [nome_produto, codigo_barras, descricao, quantidade, categoria], (err) => {
      if (err) return res.status(500).json({ mensagem: 'Erro ao cadastrar produto.', erro: err });
