...
exports.listarFornecedores = (req, res) => {
  ...
  db.query(sql, (err, results) => {
    if (err) return res.status(500).json({ mensagem: 'Erro ao buscar fornecedores.', erro: err });
...
exports.cadastrarFornecedor = (req, res) => {
  ...
  db.query(verificarCNPJ, [cnpj], (err, results) => {
    ...
    const sql = `
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `;
    db.query(sql, [nome_empresa, cnpj, endereco, telefone, email, contato_principal], (err) => {
      if (err) return res.status(500).json({ mensagem: 'Erro ao cadastrar fornecedor.', erro: err });
